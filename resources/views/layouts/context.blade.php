...
<body>
                ...
                <div class="widget-shopping-cart-content">
                    <ul class="foodix-mini-cart-list">
                        ...
                        <li class="foodix-menu-cart">
                            <a href="{{ route('cart.remove', $id) }}" class="remove-cart"><i
                                    class="far fa-trash-alt"></i></a> <!-- Use $id here -->
                            <a href="#">
                                <img src="{{ $item['image'] }}" alt="cart image">
                                {{ $item['name'] }}
                            </a>
                            <span class="quantity">{{ $item['quantity'] }} × <span><span class="currency">$</span>{{
    ...
    <script>
        window.chatbase("identify", {
            user_id: "JEzoqkIe6zckkvZagqW-K",
            user_hash: "", // this is the hash of the user_id, should be generated on the server
          });
    </script>

    <script>
        (function(){if(!window.chatbase||window.chatbase("getState")!=="initialized"){window.chatbase=(...arguments)=>{if(!window.chatbase.q){window.chatbase.q=[]}window.chatbase.q.push(arguments)};window.chatbase=new Proxy(window.chatbase,{get(target,prop){if(prop==="q"){return target.q}return(...args)=>target(prop,...args)}})}const onLoad=function(){const script=document.createElement("script");script.src="https://www.chatbase.co/embed.min.js";script.id="JEzoqkIe6zckkvZagqW-K";script.domain="www.chatbase.co";document.body.appendChild(script)};if(document.readyState==="complete"){onLoad()}else{window.addEventListener("load",onLoad)}})();
    </script>
